    ...
    <section id="skills" className="bg-black text-white px-4 py-20 text-lg flex flex-col gap-30">
      ...
      <div className="flex flex-wrap justify-between gap-y-10 px-5 sm:gap-x-10 sm:justify-center lg:gap-x-20">
        {skills.map((skill, index) => (
          <a key={index} href={skill.link} target="_blank" className="skill bg-neutral-100 rounded-4xl flex flex-col gap-5 justify-between items-center py-5 basis-[45%] relative overflow-hidden z-10 cursor-pointer group sm:basis-[20%] sm:rounded-full sm:py-17 lg:basis-[12%]">
            <img src={skill.logo} alt="languages frameworks and libraries i've learned" className="w-1/2" />
